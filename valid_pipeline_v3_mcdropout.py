...
# 可视化输出主目录与子目录
VIS_OUT_DIR = "X:/ai4eo/Shared/2025_Forge/2025/vis_output_thr015_mcdropout"
    ...
    for img_name in pbar:
        ...
        for idx, box in enumerate(dt_boxes):
            ...
            with torch.no_grad():
                ...
                # 取 10 次的平均值作为最终的 Confidence
                sh_prob = np.mean(mc_probs)
                
                # (可选) 你还可以计算标准差来看看模型有多不确定
                # sh_std = np.std(mc_probs) 
                
                # 使用你之前摸索出的 0.15 阈值，或者现在平滑后可以尝试回到 0.3
