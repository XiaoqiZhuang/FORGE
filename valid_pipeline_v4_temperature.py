...
# 可视化输出主目录与子目录
VIS_OUT_DIR = "X:/ai4eo/Shared/2025_Forge/2025/vis_output_thr015_temperature"
...
def run_visualization():
    ...
    for img_name in pbar:
        ...
        for idx, box in enumerate(dt_boxes):
            ...
            # ==========================================
            # 🚀 替换为 Temperature Scaling
            # ==========================================
            TEMPERATURE = 2.5  # 建议从 2.0 到 5.0 之间尝试
            
            with torch.no_grad():
                logits = classifier(input_tensor) # 获取原始 logit
                scaled_logits = logits / TEMPERATURE # 核心：除以温度
                
                probs = torch.softmax(scaled_logits, dim=1)
                sh_prob = probs[0, 1].item()
                
                # 由于温度软化了概率，阈值可以暂时回到一个更直观的位置
                pred_cls = 1 if sh_prob > 0.3 else 0
